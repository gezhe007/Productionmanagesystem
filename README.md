...
展示所有临期/过期批次，按状态颜色区分（红色=过期，橙色=临期）
...
为商品添加批次（输入生产日期和数量，自动计算过期日期）

调整批次数量

删除批次或商品

顶部“显示/隐藏所有操作按钮”可切换操作控件的可见性，方便日常查看
...
修改商品信息（修改后自动更新关联批次的过期日期）

删除商品（同时删除关联的货架记录和批次）

### 补货建议（Replenish）
自动计算每个货架商品当前库存与最大容量的差值，列出需补货商品

可分批添加待补货批次（生产日期、数量）

实时预览补货后库存，若超出最大容量会红色提示
...
## ⚙️ 配置说明
临期阈值：可在首页调整，保存后影响所有批次的过期状态显示

数据重置：若需清空所有数据，可在浏览器开发者工具的 Application → Local Storage 中删除对应键值，或调用 CLEAR_ALL_DATA mutation（目前未在界面暴露，可自行添加按钮）

## 📝 注意事项
所有数据仅存储在浏览器本地，清除浏览器缓存会导致数据丢失

批次的生产日期不能晚于今天（日期选择器已限制）

商品的最大容量修改时不能小于当前已有库存

删除分类或商品前请确保没有关联引用，否则会提示失败
